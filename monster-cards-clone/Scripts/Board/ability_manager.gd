...
func subscribe_card(card: CardData, player: Player):
	...
	print("subscribing card trigger: ", trigger_id)
	if is_between(0, trigger_id, Trigger.PLAYER_TRIGGER_THRESHOLD):
		triggers_to_signals[trigger_id].connect(card.run_ability)
	elif is_between(Trigger.PLAYER_TRIGGER_THRESHOLD, trigger_id, Trigger.CARD_TRIGGER_THRESHOLD):
		player.triggers_to_signals[trigger_id].connect(card.run_ability)
	elif is_between(Trigger.CARD_TRIGGER_THRESHOLD, trigger_id, Trigger.SINGLE_TIME_TRIGGER_THRESHOLD):
		card.triggers_to_signals[trigger_id].connect(card.run_ability)
